class NotifyUserUsecase():
    ...
    def __call__(self, user, sequencing_type, order_id):
        ...
        if (sequencing_type == "dna-whole-exome-sequencing"):
            return self.sms_notification_service_gateway.notify(user=user, message=f"Your order #{order_id} has been placed")
        else:
            return self.email_notification_service_gateway.notify(user=user, message=f"Your order #{order_id} has been placed")