...
def test_email_notification_service_returns_error_when_parameters_not_provided(
    gateway_under_test,
):
    assert gateway_under_test.notify(None, None) == {"errors": ["NOT_PROVIDED"]}


def test_email_notification_service_returns_error_when_email_not_provided(
    gateway_under_test,
):
...
@mock.patch.object(requests, "post")
def test_email_returns_error_when_server_response_not_200(
    mock_post, gateway_under_test
):
